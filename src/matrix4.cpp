...
Matrix4::Matrix4(float v1, float v2, float v3, float v4) noexcept
{
    entries[0] = v1;
    entries[1] = 0.0f;
    entries[2] = 0.0f;
...
void Matrix4::setDiagonal(float value) noexcept
{
    entries[0] = value;
    entries[1] = 0.0f;
    entries[2] = 0.0f;
...
Matrix4 Matrix4::inverse() const
{
    ...
    if (determinant == 0.0f)
        throw std::runtime_error("Matrix4::Matrix doesn't have its inverse");
...
void Matrix4::print() const noexcept
{
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
            std::cout << entries[4*i + j] << ' ';
        std::cout << std::endl;
    }
}
...
void Matrix4::operator+=(const Matrix4& other) noexcept
{
    for (int i = 0; i < 16; ++i)
...
Vector3 Matrix4::operator*(const Vector3& vec) const noexcept
{
...
Matrix4 Matrix4::operator*(const float value) const noexcept
{
    Matrix4 result;
...
void Matrix4::operator*=(const float value) noexcept
{
    for (int i = 0; i < 16; ++i)
        entries[i] *= value;
}
